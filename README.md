...
chrome浏览器抓包速通：右击页面，选择检查->网络，勾选保留日志，过滤选项选择Fetch/XHR，然后点击一个p站链接，随便点一个捕获到的数据包检查请求标头。如果包里没有cookie数据就换个数据包看。在数据包的请求标头里找到cookie与userAgent后，右键选择复制值，粘贴到PixivSpider.py的相应变量中。
